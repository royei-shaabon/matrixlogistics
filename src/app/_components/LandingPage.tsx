  ...
  return (
    ...
    >
      ...
      <div className="px-4 pb-4">
        <div
          className="w-full rounded-[12px] py-3 text-center text-sm font-bold"
...
const HOW_IT_WORKS = [
  {
    num: "01",
    title: "פותחים חלון הזמנה",
    desc: "המנהל מגדיר זמני פתיחה וסגירה — המערכת מתחילה לקבל הזמנות אוטומטית.",
    color: "#EFF6FF",
    numColor: "#3B82F6",
  },
  {
    num: "02",
    title: "המשתמשים מזמינים",
    desc: "כל משתמש נכנס לסביבה שלו, בוחר כמויות ושולח. מהיר וברור.",
    color: "#F0FDF4",
    numColor: "#22C55E",
  },
  {
    ...
    desc: "המערכת מרכזת את כל הנתונים. המנהל מקבל דוח מסכם מוכן לייצוא.",
    color: "#FFF7ED",
    numColor: "#F59E0B",
  },
];
    ...
    <div dir="rtl" style={{ background: "#F8FAFC", fontFamily: "system-ui, -apple-system, sans-serif" }}>
        ...
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "#3B82F6" }}>
              <svg width="16" height="16" fill="white" viewBox="0 0 24 24"><path d="M20 7H4a2 2 0 00-2 2v10a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2zM4 5h16V4a1 1 0 00-1-1H5a1 1 0 00-1 1v1z"/></svg>
            </div>
            <span className="font-bold text-base" style={{ color: "#1E293B" }}>Matrix Supply</span>
          </div>

          {/* Desktop nav */}
          <nav className="hidden md:flex items-center gap-3">
            <Link href="/faq" className="text-sm font-medium px-3 py-1.5 rounded-lg transition-colors" style={{ color: "#64748B" }}>שאלות נפוצות</Link>
            <Link href="/guide" className="text-sm font-medium px-3 py-1.5 rounded-lg transition-colors" style={{ color: "#64748B" }}>מדריך שימוש</Link>
            <Link href="/login" className="text-sm font-medium px-4 py-2 rounded-xl border transition-colors" style={{ color: "#1E293B", borderColor: "#CBD5E1" }}>התחברות</Link>
            <Link href="/register" className="text-sm font-bold px-4 py-2 rounded-xl transition-colors" style={{ background: "#3B82F6", color: "#FFFFFF" }}>הרשמה חינם</Link>
          </nav>
        ...
        {menuOpen && (
          <div className="md:hidden px-4 pb-4 space-y-2" style={{ borderTop: "1px solid #E2E8F0", background: "#FFFFFF" }}>
            ...
            <div className="flex gap-2 pt-1">
              <Link href="/login" className="flex-1 text-center py-2 rounded-xl border text-sm font-medium" style={{ color: "#1E293B", borderColor: "#CBD5E1" }}>התחברות</Link>
              <Link href="/register" className="flex-1 text-center py-2 rounded-xl text-sm font-bold" style={{ background: "#3B82F6", color: "#FFFFFF" }}>הרשמה</Link>
            </div>
          </div>
        )}
      </header>
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-lg mb-8" style={{ color: "#64748B", lineHeight: 1.7 }}>
              פתיחת חלונות הזמנה, איסוף כמויות, ניהול משתמשים ודוחות מסכמים — במקום אחד. בלי וואטסאפ, בלי אקסלים.
            </p>
            <div className="flex gap-3 flex-wrap">
              <Link
                href="/register"
                className="px-6 py-3 rounded-xl font-bold text-white text-base transition-all"
                style={{ background: "#3B82F6", boxShadow: "0 4px 14px rgba(59,130,246,0.4)" }}
              >
                התחל עכשיו — חינם
              </Link>
              <a
                href="#how"
                className="px-6 py-3 rounded-xl font-semibold text-base transition-all border"
                style={{ color: "#1E293B", borderColor: "#CBD5E1", background: "#FFFFFF" }}
              >
                איך זה עובד?
              </a>
            </div>
        ...
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {WHO_FOR.map((item) => (
              <div
                key={item}
                className="rounded-[16px] px-5 py-4 text-center text-sm font-semibold"
                style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", color: "#1E293B", boxShadow: "0 2px 8px rgba(15,23,42,0.05)" }}
              >
                {item}
              </div>
            ))}
      ...
      <section className="py-20" style={{ background: "#FFFFFF" }}>
        <div className="max-w-5xl mx-auto px-4">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl font-black mb-8" style={{ color: "#1E293B" }}>יתרונות מרכזיים</h2>
            ...
            <div className="rounded-[24px] p-8 text-center" style={{ background: "#F8FAFC", border: "1px solid #E2E8F0" }}>
              <p className="text-6xl font-black mb-2" style={{ color: "#3B82F6" }}>0₪</p>
              <p className="text-lg font-bold mb-3" style={{ color: "#1E293B" }}>בחינם לחלוטין</p>
              <p className="text-sm mb-6" style={{ color: "#64748B" }}>אין עלויות נסתרות. אין צורך בכרטיס אשראי. מתחילים מיד.</p>
              <Link
                href="/register"
      ...
      <section className="py-20" style={{ background: "#1E293B" }}>
        <div className="max-w-5xl mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-4xl font-black mb-4" style={{ color: "#F1F5F9" }}>מוכנים להתחיל?</h2>
          <p className="text-base mb-8" style={{ color: "#94A3B8" }}>
            צרו סביבה חדשה תוך דקה, או הצטרפו עם קוד הזמנה שקיבלתם ממנהל הסביבה.
          </p>
          <div className="flex gap-3 justify-center flex-wrap">
            ...
            <Link
              href="/login"
              className="px-8 py-3 rounded-xl font-semibold text-base border transition-all"
        ...
        <div className="max-w-5xl mx-auto px-4 py-10 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-md flex items-center justify-center" style={{ background: "#3B82F6" }}>
              <svg width="12" height="12" fill="white" viewBox="0 0 24 24"><path d="M20 7H4a2 2 0 00-2 2v10a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2z"/></svg>
            </div>
            <span className="text-sm font-bold" style={{ color: "#64748B" }}>Matrix Supply Order</span>
          </div>
